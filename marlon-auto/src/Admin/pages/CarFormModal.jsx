...
export default function CarFormModal({
  ...
  useEffect(() => {
    ...
    if (car?.id) {
      setForm({
        ...
        price_eur:
          car.price_eur !== null &&
          car.price_eur !== undefined
            ? String(car.price_eur)
            : "",
        category:
          car.category ??
          (String(car.fuel_type || "").includes("electric")
            ? "electric"
            : "regular"),
        status:
          car.status ??
          (car.is_published ? "active" : "inactive"),
      });
    } else {
      setForm(empty);
    }
  ...
  async function onSubmit(e) {
    ...
    const price = Number(form.price_eur);
    ...
    const payload = {
      ...
      torque_nm:
        form.torque_nm === ""
          ? null
          : Number(form.torque_nm),

      consumption_l_100:
        form.consumption_l_100 === ""
          ? null
          : Number(form.consumption_l_100),

      range_km:
        form.range_km === ""
          ? null
          : Number(form.range_km),

      battery_kwh:
        form.battery_kwh === ""
          ? null
          : Number(form.battery_kwh),

      charging_kw:
        form.charging_kw === ""
          ? null
          : Number(form.charging_kw),
